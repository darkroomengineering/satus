...
- `useWebGLRect` is now the one DOM-to-WebGL hook, and it runs on the DOM side: `const [setRectRef, rect] = useWebGLRect(onUpdate, options)` measures the element with hamo's `useRect` (the options are its options, e.g. `ignoreTransform` for an element under a `TransformProvider`, whose translate the hook adds itself, horizontally too) and calls `onUpdate` on Lenis scroll events, provider transform changes and re-measures, with the element's position, scale and visibility in camera units. A third return value, `update`, places a mesh that mounts later than its element, such as everything behind the after-load canvas gate, from the mesh's ref callback. Visibility comes from the scroll math; `useWebGLElement` and its IntersectionObserver are removed. The WebGL `Image` takes a `meshRef` instead of `rect`/`visible`.
- Lenis stops its inertia when an internal link is clicked (`stopInertiaOnNavigate`) and allows nested scroll containers (`allowNestedScroll`).
