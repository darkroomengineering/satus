...
- Next.js 16.3 preview: bumped `next` to the `preview` dist-tag, enabled top-level `partialPrefetching: true` (the 16.3 successor to `experimental.prefetchInlining`), removed the now-superseded `experimental.prefetchInlining` flag from the `experimental` block, and replaced the placeholder `app/loading.tsx` with an accessible skeleton shell (`role="status"`, `aria-busy="true"`, `sr-only` label, `animate-pulse` bars) demonstrating the instant-navigation loading pattern.
