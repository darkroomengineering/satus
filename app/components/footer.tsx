import { Link } from "@/components/link";

export function Footer() {
  return (
    <footer className="flex dt:flex-row flex-col dt:items-end items-center justify-between p-safe font-mono uppercase">
      <Link href="https://darkroom.engineering/" className="link" aria-label="Darkroom Engineering">
        darkroom.engineering
      </Link>
      <div>
        <Link href="https://github.com/darkroomengineering/satus/generate" className="link">
          use this template
        </Link>
        {" / "}
        <Link href="https://github.com/darkroomengineering/satus" className="link">
          github
        </Link>
      </div>
    </footer>
  );
}
