...
| M13 | med | components | `preload` prop's JSDoc/README misdescribe it — **correction (2026-07-09, verified against next@16.2.9 source during remediation):** `preload` IS Next's native LCP mechanism there (`priority` is deprecated; setting both throws), and the wrapper already forwards it. The finding survives as stale docs only; the audit's original "wire it to priority" direction was wrong and would have crashed at runtime | `components/ui/image/index.tsx:119,138` | CONFIRMED (reclassified) |
