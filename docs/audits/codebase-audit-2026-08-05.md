...
| H2  | High   | CMS/SEO    | CMS `page`/`article` docs sitemapped + llms.txt'd with zero rendering route — 404 trap             | `lib/seo/routes.ts:84-139`, `lib/integrations/sanity/utils/link.ts:39-42`   | CONFIRMED (×2 agents + Codex)           |
...
| H4  | High   | Sanity     | `SANITY_STUDIO_PROJECT_ID`-only config breaks frontend AND `/studio` (hydration 404)               | `lib/utils/validation.ts:22-29`, `lib/integrations/sanity/env.ts:29-32`     | CONFIRMED (scenario corrected by Codex) |
...
| L10 | Low    | SEO        | Dead exports: `articleSchema`/`breadcrumbSchema`/`articleQuery` have zero callers                  | `lib/seo/schemas.ts:55-93`, `lib/integrations/sanity/queries.ts:39`         | CONFIRMED                               |
