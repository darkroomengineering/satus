...
export function useScrollTrigger(
  ...
  const [elementEndKeyword, viewportEndKeyword] = typeof end === "string" ? end.split(" ") : [end];

  let viewportStart = isNumber(viewportStartKeyword) ? Number.parseFloat(viewportStartKeyword) : 0;
  ...
  let viewportEnd = isNumber(viewportEndKeyword) ? Number.parseFloat(viewportEndKeyword) : 0;
  ...
  if (elementStartKeyword === "center") elementStart = (rect?.top || 0) + (rect?.height || 0) * 0.5;
  ...
  if (elementEndKeyword === "center") elementEnd = (rect?.top || 0) + (rect?.height || 0) * 0.5;
  ...
  const [setProgress, _getProgress] = useLazyState<number>(
    ...
    (progress: number, lastProgress: number | undefined) => {
      ...
      if ((progress >= 0 && lastProgress < 0) || (progress <= 1 && lastProgress > 1)) {
