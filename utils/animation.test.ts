...
describe("stagger", () => {
  ...
  it("should return identical values for all elements when staggerAmount is 0", () => {
    ...
    const results = Array.from({ length: total }, (_, i) => stagger(i, total, progress, 0));
...
describe("spring", () => {
  ...
  it("should produce more oscillation with low damping", () => {
    ...
    for (let i = 0; i < 600; i++) {
      ...
      if ((prev < target && current > target) || (prev > target && current < target)) {
        overshootCount++;
      }
    }
...
describe("fromTo", () => {
  ...
  it("should apply stagger to elements", () => {
    ...
    expect(results[0]!.values.value).toBeGreaterThanOrEqual(results[4]!.values.value!);
  });
