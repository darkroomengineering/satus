...
export const easings = {
  ...
  easeInOutCubic: (x: number): number => (x < 0.5 ? 4 * x * x * x : 1 - pow(-2 * x + 2, 3) / 2),
  ...
  easeInOutQuart: (x: number): number => (x < 0.5 ? 8 * x * x * x * x : 1 - pow(-2 * x + 2, 4) / 2),
  ...
  easeOutBack: (x: number): number => 1 + c3 * pow(x - 1, 3) + c1 * pow(x - 1, 2),
