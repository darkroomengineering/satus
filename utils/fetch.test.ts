...
describe("fetchWithTimeout", () => {
  ...
  it("should abort when timeout is exceeded", async () => {
    await expect(fetchWithTimeout(`${baseURL}/slow`, { timeout: 100 })).rejects.toThrow();
  });
...
describe("fetchJSON", () => {
  it("should parse JSON response automatically", async () => {
    const data = await fetchJSON<{ message: string; count: number }>(`${baseURL}/json`);
    expect(data.message).toBe("hello");
    expect(data.count).toBe(42);
  });
  ...
  it("should throw on 404 status", async () => {
    await expect(fetchJSON(`${baseURL}/error-404`)).rejects.toThrow("HTTP 404: Not Found");
  });

  it("should respect timeout option", async () => {
    await expect(fetchJSON(`${baseURL}/slow`, { timeout: 100 })).rejects.toThrow();
  });
